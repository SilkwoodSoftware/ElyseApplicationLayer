...
import { ReadDocRblistNamesService } from './read-doc-rblist-names.service';
...
@Component({
  selector: 'app-read-doc-rblist-names',
  templateUrl: './read-doc-rblist-names.component.html',
  styleUrls: ['./read-doc-rblist-names.component.scss']
})
export class ReadDocRblistNamesComponent implements OnInit {
  docRblistNames: any[] = [];
  selectedRows: any[] = [];
  sortColumn: string = 'ListPosition';
  ...
  private getAllDocRblistNamesSubscription: Subscription | null = null;
  private isDragging: boolean = false;
  private mouseDownPosition: { x: number, y: number } | null = null;
  private dragStartPosition: { x: number, y: number } | null = null;
  private docRblistAddedSubscription: Subscription | null = null;
  ...
  ngOnInit(): void {
    this.fetchAllDocRblistNames();
    this.docRblistRefreshService.refresh$.subscribe(() => {
      this.fetchAllDocRblistNames();
    });
    this.docRblistAddedSubscription = this.selectedIdsService.docRblistAdded$.subscribe(() => {
     this.fetchAllDocRblistNames();
    });
  }

  fetchAllDocRblistNames(): void {
    console.log('Making HTTP request to fetch all document radio button list names.');
    this.getAllDocRblistNamesSubscription = this.readDocRblistNamesService.getAllDocRblistNames().subscribe(
      (response) => {
        this.docRblistNames = [...response.docRblistNames];        
        this.result = {
          numberOfRows: response.docRblistNames.length,           
          transactionMessage: response.transactionMessage,
          transactionStatus: response.transactionStatus,
        };
        console.log('Document radio button list names fetched, updating docRblistNames property.');
      ...
      (error) => {
        console.error('Error fetching document radio button list names:', error);
      }
    );
  }
  ...
  private updateMessageItems(data: any): void {
    this.messageItems = [
      ...
      { label: this.messageLabelService.getLabel('numberOfRows'), value: data.docRblistNames.length.toString() }
    ];
  }
    ...
    if (!this.isDragging) {
      if (event.ctrlKey || event.metaKey) {
        ...
        if (index > -1) {
          ...
          this.selectedNames.push(row.Name);
        }
      } else if (event.shiftKey && this.selectedRows.length > 0) {
        const lastSelectedIndex = this.docRblistNames.findIndex(docRblistName => docRblistName === this.selectedRows[this.selectedRows.length - 1]);
        const currentSelectedIndex = this.docRblistNames.findIndex(docRblistName => docRblistName === row);
        ...
        this.selectedRows = this.docRblistNames.slice(startIndex, endIndex + 1);
        this.selectedIds = this.selectedRows.map(selectedRow => selectedRow.ID);
        this.selectedNames = this.selectedRows.map(selectedRow => selectedRow.Name);
      } else {
  ...
  onColumnSelect(column: string): void {
    ...
    this.sortDocRblistNames();
  }

  sortDocRblistNames(): void {
    this.docRblistNames.sort((a, b) => {
  ...
  ngOnDestroy(): void {
    if (this.getAllDocRblistNamesSubscription) {
      this.getAllDocRblistNamesSubscription.unsubscribe();
      this.getAllDocRblistNamesSubscription = null;
    }
    if (this.docRblistAddedSubscription) {
      this.docRblistAddedSubscription.unsubscribe();
      this.docRblistAddedSubscription = null;
    }
  }
