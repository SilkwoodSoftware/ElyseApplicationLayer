...
@Component({
  selector: 'app-create-document-id-dialog',
  templateUrl: './create-document-id-dialog.component.html',
  styleUrls: ['../../shared/stylesheets/dialogs.scss'],
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule]
})
export class CreateDocumentIdDialogComponent {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<CreateDocumentIdDialogComponent>,
    private createDocumentIdService: CreateDocumentIdService
  ) {
  ...
  onSubmit() {
    const documentId = this.createForm.get('documentId')?.value;
    this.createDocumentIdService.createDocumentId(documentId).subscribe(
      result => {
