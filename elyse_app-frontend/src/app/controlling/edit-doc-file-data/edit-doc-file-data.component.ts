...
export class EditDocFileDataComponent implements OnInit, OnDestroy {
        ...
        .subscribe(
         (response) => {
          ...
          formValuesData.forEach((item: any) => {
            ...
            // If Name ID Parameter is empty, use only Field Type as key since these are
            // singleton fields (Transaction Group, Lock Status, Filename, File Release ID)
            // and the field name may differ between formValuesData and allFormFieldsData
          ...
          allFormFieldsData.forEach((fieldDef: any) => {
            ...
            // Create matching key - must use same logic as valuesMap above
            const uniqueId = hasNameIdParam
  ...
  prepareField(field: FieldConfig): FieldConfig {
    ...
    } else if (routingInfo && routingInfo['Data Type'] === 'Lookup') {
      // Lookup fields use progressive discovery via form field menu
      field.isLookup = true;
      field.lookupMenuId = routingInfo['Lookup Menu ID'];
      field.lookupIdAlias = routingInfo['Lookup ID Alias'];
      field.lookupNameAlias = routingInfo['Lookup Name Alias'];
  ...
  private collectChangedFields(): { field: FieldConfig, action: string, displayAction: string }[] {
    ...
    allFields.forEach(field => {
      if (field.isLookup) {
        // Lookup fields: detect changes by comparing attributeId (the selected ID)
        if (field.attributeId !== field.originalAttributeId) {
          const routingInfo = this.formFieldRoutingService.getRoutingInfo(field.fieldType);
          if (field.existsInDatabase) {
            // Update existing value (uses Create route as upsert)
            changedFields.push({
              field,
              action: 'update',
              displayAction: this.actionDisplayMap['update'] || 'update'
            });
          } else if (field.attributeId !== null && field.attributeId !== undefined &&
