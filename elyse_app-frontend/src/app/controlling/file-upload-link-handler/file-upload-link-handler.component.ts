...
@Component({
  selector: 'app-file-upload-link-handler',
  template: `
...
export class FileUploadLinkHandlerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (this.documentId) {
        ...
        this.validateAndUpload(this.documentId);
      } else {
  ...
  private validateAndUpload(documentId: string): void {
    this.documentValidationService.validateDocumentId(documentId).subscribe(
      (response) => {
        if (response.validationResult === 'Pass') {
          // Document ID is valid, open file selection dialog
          this.openFileSelectionDialog(documentId);
        } else {
