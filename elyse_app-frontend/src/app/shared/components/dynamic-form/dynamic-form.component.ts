...
import { Component, Input, Output, EventEmitter, OnChanges, SimpleChanges, OnInit, ChangeDetectorRef, ViewContainerRef, ViewChild } from '@angular/core';
...
export class DynamicFormComponent implements OnChanges, OnInit {
  ...
  constructor(
    ...
    private _formFieldMenuService: FormFieldMenuService  // Eagerly instantiate to preload menu CSV data for Lookup fields
  ...
  onAction(field: FieldConfig, action: string, event?: MouseEvent) {
    ...
    switch (action) {
      ...
      case 'delete':
        ...
        break;
      case 'add':
        // For Lookup fields, the Add button opens the form field menu for progressive discovery
        if (field.isLookup && event) {
          this.openLookupMenu(field, event);
          return;
        }
      ...
      case 'revert':
        ...
        if (control) {
          if (field.isLookup) {
            // For Lookup fields, revert clears the pre-populated value
      ...
      private updateFieldState(field: FieldConfig, action: string) {
        switch (action) {
          ...
          case 'update':
            ...
            if (control) {
              if (field.isLookup) {
                // For Lookup fields, update both the display value and the attribute ID
                field.originalValue = field.value;
                field.originalAttributeId = field.attributeId;
              } else if (field.isDropdown) {
  ...
  /**
   * Open the form field menu for Lookup fields (progressive discovery).
   * Creates a FormFieldMenuComponent dynamically, which handles the menu display,
   * form/table navigation, and returns the selected {id, name} pair.
   */
  openLookupMenu(field: FieldConfig, event: MouseEvent): void {
    if (!field.lookupMenuId) {
      console.error(`No lookupMenuId configured for Lookup field: ${field.displayName || field.name}`);
      return;
    }
    ...
    setTimeout(() => {
      try {
        ...
        componentInstance.formFieldMenuId = field.lookupMenuId || '';
        componentInstance.position = position;
        componentInstance.idAlias = field.lookupIdAlias;
        componentInstance.nameAlias = field.lookupNameAlias;
        ...
        componentInstance.onValueSelected = (value: any, displayValue: any) => {
          if ((value === null || value === undefined) && displayValue !== undefined) {
            console.error(`Lookup field ${field.name} received null/undefined value with displayValue: ${displayValue}`);
            this.dialog.open(ApplicationErrorDialogComponent, {
              data: {
                title: 'Field Selection Error',
                errors: [{
                  fieldName: field.displayName || field.name,
                  errorMessage: 'The field returned a display value but no ID value. This indicates a configuration error.'
                }]
              }
            });
            return;
          ...
          // Update the form control with the display value
          const control = this.form.get(field.name);
          if (control) {
        ...
        console.error('Error creating lookup menu component:', error);
      }
    }, 0);
  }
  ...
  isAddButtonActive(field: FieldConfig): boolean {
    ...
    // Lookup fields: Add button is always enabled to allow opening the menu
    // for both entering a new value and replacing an existing one
    if (field.isLookup) {
      return true;
    }
  ...
  isSaveButtonActive(field: FieldConfig): boolean {
    ...
    // Lookup fields: Save (Update field) button is active when value has changed
    if (field.isLookup) {
