...
export class HelpIndexComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    // Add click listener to intercept internal help links (fragment identifiers like #help:filename.md)
    this.clickListener = (e: Event) => {
      ...
      if (anchor) {
        ...
        if (href && href.startsWith('#help:')) {
          e.preventDefault();
          const contentFile = href.substring(6); // Remove '#help:' prefix
          this.navigateToHelpByContentFile(contentFile);
        }
      }
    };
  ...
  /**
   * Navigate to a help topic by its content file name (used by internal .md links).
   *
   * The same content file (e.g. document-groups.md) can appear under multiple
   * parent topics in the help directory, each with different role restrictions.
   * To ensure inline links always resolve — even when the target topic is not
   * present in the user's role-filtered menu — the search is performed against
   * the full, unfiltered help tree first, falling back to the filtered tree
   * for menu highlighting when possible.
   */
  private navigateToHelpByContentFile(contentFile: string): void {
    // 1. Try the role-filtered tree first (preferred — allows menu highlighting)
    let item = this.findHelpItemByContentFile(this.helpItems, contentFile);

    if (item) {
      this.selectHelpItem(item);
      this.expandParentsOf(contentFile, this.helpItems);
      return;
    }

    // 2. Search the full unfiltered tree so cross-role links still work
    const allItems = this.helpMenuService.getAllHelpItems();
    item = this.findHelpItemByContentFile(allItems, contentFile);

    if (item) {
      this.selectHelpItem(item);
      // The item may not be in the filtered sidebar, so menu highlighting
      // won't apply, but the content is still displayed correctly.
    } else {
      console.warn(`Help topic not found for content file: ${contentFile}`);
    }
  }

  /**
   * Recursively find a help item by its content file name
   */
  private findHelpItemByContentFile(items: HelpMenuItem[], contentFile: string): HelpMenuItem | null {
    for (const item of items) {
      if (item.contentFile === contentFile) {
        return item;
      }
      if (item.children && item.children.length > 0) {
        const found = this.findHelpItemByContentFile(item.children, contentFile);
        if (found) {
          return found;
        }
      }
  ...
  /**
   * Expand parent items that contain a child with the given content file
   */
  private expandParentsOf(contentFile: string, items: HelpMenuItem[]): boolean {
    for (const item of items) {
      if (item.contentFile === contentFile) {
        return true;
      }
      if (item.children && item.children.length > 0) {
        if (this.expandParentsOf(contentFile, item.children)) {
          item.expanded = true;
          return true;
        }
      }
  ...
  private convertMarkdownToHtml(markdown: string): string {
    ...
    try {
      ...
      //   - Internal .md links: rewrite href to use #help: fragment for click interception
      //   - External links: add target="_blank" so they don't navigate the help popup away
      html = html.replace(/<a\s+href="([^"]*)"([^>]*)>/g, (match: string, href: string, rest: string) => {
        if (href.endsWith('.md')) {
          return `<a href="#help:${href}"${rest}>`;
        }
