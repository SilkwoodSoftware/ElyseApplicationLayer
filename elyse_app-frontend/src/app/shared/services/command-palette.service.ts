...
import { COMMAND_PALETTE_SCHEMA } from '../utils/csv-schemas';
...
export class CommandPaletteService {
  ...
  private parseCommandsCsv(csv: string): CommandPaletteCommand[] {
    const result = validateCsv(csv, COMMAND_PALETTE_SCHEMA);

    this.csvDiagnosticService.reportErrors(
      COMMAND_PALETTE_SCHEMA.fileName, result.errors, result.validRows.length, result.rejectedRowCount
    );
    ...
    return result.validRows.map(row => {
      ...
      return {
        ...
        Shortcut: row['Shortcut'].trim(),
        Description: row['Description'].trim(),
        Icon: row['Icon'].trim(),
        Actions: row['Actions'].trim(),
        Objects: row['Objects'].trim(),
