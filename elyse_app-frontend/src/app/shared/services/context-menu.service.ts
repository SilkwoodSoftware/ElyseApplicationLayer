...
import { CONTEXT_MENUS_SCHEMA } from '../utils/csv-schemas';
...
export interface ContextMenuItem {
  ...
  actionType: 'READ' | 'DUC' | 'FORM' | 'CUSTOM' | 'CHAIN';
  reference: string;
...
export class ContextMenuService {
  ...
  private parseContextMenusCsv(csv: string): ContextMenuItem[] {
    const result = validateCsv(csv, CONTEXT_MENUS_SCHEMA);

    this.csvDiagnosticService.reportErrors(
      CONTEXT_MENUS_SCHEMA.fileName, result.errors, result.validRows.length, result.rejectedRowCount
    );

    return result.validRows.map(row => {
      const parentMenuId = row['ParentMenu']?.trim() || undefined;

      // Default visibilityConstraint to ALWAYS when empty
      let visibilityConstraint: 'ALWAYS' | 'SINGLE_ROW' | 'ANY_ROWS' = 'ALWAYS';
      const rawVisibility = row['VisibilityConstraint']?.trim();
      if (rawVisibility === 'SINGLE_ROW' || rawVisibility === 'ANY_ROWS') {
        visibilityConstraint = rawVisibility;
      }

      // Map actionType faithfully — default to READ only when empty
      let actionType: 'READ' | 'DUC' | 'FORM' | 'CUSTOM' | 'CHAIN' = 'READ';
      const rawAction = row['ActionType']?.trim();
      if (rawAction === 'DUC' || rawAction === 'FORM' || rawAction === 'CUSTOM' || rawAction === 'CHAIN') {
      ...
      return {
        ...
        label: row['Label'],
        parentMenuId,
        roles: row['Roles'] ? row['Roles'].split(';').map(role => role.trim()).filter(role => role.length > 0) : [],
        visibilityConstraint,
        actionType,
        reference: row['Reference'],
        keyboardShortcut: row['Shortcut'],
