...
import { FORM_CHAINS_SCHEMA } from '../utils/csv-schemas';
...
export class CsvFormChainService {
  ...
  private parseFormChainsCsv(csv: string): CsvFormChainLink[] {
    const result = validateCsv(csv, FORM_CHAINS_SCHEMA);

    this.csvDiagnosticService.reportErrors(
      FORM_CHAINS_SCHEMA.fileName, result.errors, result.validRows.length, result.rejectedRowCount
    );

    return result.validRows.map(row => ({
      chainId: row['ChainId'],
      linkId: parseInt(row['LinkId']) || 0,
      actionType: row['ActionType'],
      reference: row['Reference'],
      condition: row['Condition'] || undefined,
      inputParameters: row['InputParameters'] || undefined,
      outputParameters: row['OutputParameters'] || undefined,
