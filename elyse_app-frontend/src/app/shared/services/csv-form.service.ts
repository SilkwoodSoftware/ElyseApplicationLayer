...
import { FORMS_SCHEMA, FORM_FIELDS_SCHEMA, FORM_FORMFIELD_LINKS_SCHEMA, INPUT_ID_PARAMETER_MAPPING_SCHEMA } from '../utils/csv-schemas';
...
export class CsvFormService {
  ...
  private loadFormFieldLinks(): void {
    ...
    private parseParameterIdDataTypesCsv(csv: string): Array<{parameterId: string, dataType: string}> {
      ...
      return result.validRows.map(row => ({
        parameterId: row['InputParameter'],
        dataType: row['IdDataType']
      }));
    }
  ...
  private parseFormsCsv(csv: string): CsvFormDefinition[] {
    const result = validateCsv(csv, FORMS_SCHEMA);

    this.csvDiagnosticService.reportErrors(
      FORMS_SCHEMA.fileName, result.errors, result.validRows.length, result.rejectedRowCount
    );
    ...
    return result.validRows.map(row => ({
      ...
      title: row['Title'],
      titleData: row['TitleData'] || undefined,
  ...
  private parseFormFieldsCsv(csv: string): CsvFormField[] {
    const result = validateCsv(csv, FORM_FIELDS_SCHEMA);

    this.csvDiagnosticService.reportErrors(
      FORM_FIELDS_SCHEMA.fileName, result.errors, result.validRows.length, result.rejectedRowCount
    );
    ...
    return result.validRows.map(row => ({
      ...
      characterLimit: row['CharacterLimit'] ? parseInt(row['CharacterLimit']) || undefined : undefined,
      dataType: row['DataType'],
      defaultValue: row['DefaultValue'] || undefined,
      populationType: row['PopulationType'],
      reference: row['Reference'] || undefined,
      parameterId: row['ParameterId'] || undefined,
      placeholder: row['Placeholder'] || undefined,
      helpText: row['HelpText'] || undefined,
      idAlias: row['IdAlias'] || undefined,
      nameAlias: row['NameAlias'] || undefined
    }));
  }
  ...
  private parseFormFieldLinksCsv(csv: string): FormFieldLink[] {
    const result = validateCsv(csv, FORM_FORMFIELD_LINKS_SCHEMA);

    this.csvDiagnosticService.reportErrors(
      FORM_FORMFIELD_LINKS_SCHEMA.fileName, result.errors, result.validRows.length, result.rejectedRowCount
    );

    return result.validRows.map(row => ({
      formId: row['FormId'],
      formFieldId: row['FormFieldId'],
      isMandatory: row['IsMandatory'] === 'Y',
