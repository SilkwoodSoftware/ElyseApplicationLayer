...
import { CUSTOM_VIEWS_SCHEMA } from '../utils/csv-schemas';
...
export class CustomViewConfigService extends BaseRouteConfigService<CustomViewConfig> {
    ...
    protected parseCsvData(csvData: string): CustomViewConfig[] {
        const result = validateCsv(csvData, CUSTOM_VIEWS_SCHEMA);

        this.csvDiagnosticService.reportErrors(
            CUSTOM_VIEWS_SCHEMA.fileName, result.errors, result.validRows.length, result.rejectedRowCount
        );

        return result.validRows.map(row => ({
            viewId: row['ViewId'].trim(),
            storedProcedure: row['StoredProcedure'].trim(),
            routeUrl: row['RouteUrl'].trim(),
            apiEndpoint: row['ApiEndpoint'].trim(),
            viewType: row['ViewType'].trim() as CustomViewType,
            templateName: row['TemplateName']?.trim() || '',
            displayName: row['Title'].trim(),
            ...
            chainId: row['ChainId']?.trim() || undefined,
