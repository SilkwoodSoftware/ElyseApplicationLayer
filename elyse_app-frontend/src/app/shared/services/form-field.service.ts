...
export class FormFieldService {
  ...
  private parseFieldMappingsCsv(csv: string): DropdownFieldMapping[] {
    ...
    return result.validRows
      .filter(row => row['InputParameter'].trim() && row['IDNameAlias'].trim() && row['NameFieldAlias'].trim())
      .map(row => ({
        inputParameter: row['InputParameter'].trim(),
        idFieldName: row['IDNameAlias'].trim(),
        nameFieldName: row['NameFieldAlias'].trim(),
      }));
  }
