...
import { HELP_DIRECTORY_SCHEMA } from '../utils/csv-schemas';
...
export class HelpMenuService {
  ...
  private parseHelpDirectory(csvContent: string): HelpMenuItem[] {
    const result = validateCsv(csvContent, HELP_DIRECTORY_SCHEMA);

    this.csvDiagnosticService.reportErrors(
      HELP_DIRECTORY_SCHEMA.fileName, result.errors, result.validRows.length, result.rejectedRowCount
    );

    return result.validRows.map(row => ({
      helpId: row['HelpId'],
      title: row['Title'],
      parentHelpId: row['ParentHelpId'] || undefined,
