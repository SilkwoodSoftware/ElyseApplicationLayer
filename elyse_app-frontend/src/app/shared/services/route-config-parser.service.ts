...
import { READ_ROUTES_SCHEMA } from '../utils/csv-schemas';
...
export class RouteConfigParserService {
  ...
  private parseRoutesCSV(csv: string): RouteConfig[] {
    const result = validateCsv(csv, READ_ROUTES_SCHEMA);

    this.csvDiagnosticService.reportErrors(
      READ_ROUTES_SCHEMA.fileName, result.errors, result.validRows.length, result.rejectedRowCount
    );
