...
import { ActionType } from '../models/menu-item.model';
  ...
  describe('convertForestToComponentModel', () => {
    it('should convert a valid MenuForest to a MenuComponentModel', () => {
      ...
      if (E.isRight(result)) {
        ...
        expect(firstItem.type).toBe('parent');
        if (firstItem.type === 'parent') {
          expect(firstItem.children.length).toBe(1);
          expect(firstItem.children[0].label).toBe('Submenu 1');
        }
      }
    });
