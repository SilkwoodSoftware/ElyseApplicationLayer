...
In the [Command Palette](command-palette.md) enter `Create` into the **Actions** field and `Document` into the **Objects** field and select the command **Create and Edit Document ID (Editor Role)**.
...
Editor role privileges allows the loading of files.  See the [Loading Files](loading-files.md) section.

## Editing Metadata

In the [Command Palette](command-palette.md), enter `Edit` into the **Actions** field to display the commands for editing documents or files.  The edit form for a document or a file can also be opened via a context menu.  From a table of documents or files, select a single row, right click and then navigate to **Edit Document Data** or **Edit File Data**.
...
A user with Editor level role permissions can book a document in and out.  In the Command Palette, enter `Booking` into the **Objects** field to display a list of all booking related commands.  See the [Booking Management](booking-management.md) section for more details.
...
See the [Workflow Management](workflow-management.md) section for more details. 
