...
public class Startup
{
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        ...
        // Configure per-user token bucket rate limiting
        // Reads configuration from appsettings.json "RateLimiting" section
        var rateLimitSection = Configuration.GetSection("RateLimiting");
        var tokenLimit = rateLimitSection.GetValue<int>("TokenLimit", 1000);
        var tokensPerPeriod = rateLimitSection.GetValue<int>("TokensPerPeriod", 100);
        var replenishmentPeriodSeconds = rateLimitSection.GetValue<int>("ReplenishmentPeriodSeconds", 10);
        ...
        services.AddRateLimiter(options =>
        {
            ...
            {
                // Key on the KCD-authenticated Windows identity (e.g. DOMAIN\username)
                // Falls back to "anonymous" if no authenticated user (should not occur in production)
            ...
            {
                context.HttpContext.Response.Headers["Retry-After"] = replenishmentPeriodSeconds.ToString();
    ...
    public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
    {
        ...
        app.UseRateLimiter(); // Per-user token bucket rate limiting — must be after auth so Windows identity is available
        app.UseEndpoints(endpoints =>
        {
            endpoints.MapControllers().RequireRateLimiting("per-user");
        });
    }
}
