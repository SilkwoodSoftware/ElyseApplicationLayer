...
// Returns metadata for documents and associated files where a date record is before a given date.
// Doesn't apply a filter group.
[Route("api/document/date/before")]
[ApiController]
public class ReadAllDocumentsDateBefore : BaseStoredProcedureController
{
    public ReadAllDocumentsDateBefore(StoredProcedureService storedProcedureService, ILogger<ReadAllDocumentsDateBefore> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> Read([FromQuery] long? docDateNameId = null, [FromQuery] string dateValue = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading all documents date before",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_docs_date_before", parameters);
            },
            result =>
            {
