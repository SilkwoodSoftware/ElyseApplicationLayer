...
// Returns metadata for documents and associated files where a date record is equal to a given date.
// Does not apply a filter group.
[Route("api/document/date/equals")]
[ApiController]
public class ReadAllDocumentsDateEquals : BaseStoredProcedureController
{
    public ReadAllDocumentsDateEquals(StoredProcedureService storedProcedureService, ILogger<ReadAllDocumentsDateEquals> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? docDateNameId = null, [FromQuery] string dateValue = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading all documents date equals",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_docs_date_equals", parameters);
            },
            result =>
            {
