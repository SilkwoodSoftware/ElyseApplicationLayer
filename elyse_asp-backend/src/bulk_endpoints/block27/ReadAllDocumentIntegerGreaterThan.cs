...
// Returns metadata for documents and associated files where the designated integer field is greater than the given value.
// No filter group is applied.
[Route("api/document/integer/greater-than")]
[ApiController]
public class ReadAllDocumentIntegerGreaterThan : BaseStoredProcedureController
{
    public ReadAllDocumentIntegerGreaterThan(StoredProcedureService storedProcedureService, ILogger<ReadAllDocumentIntegerGreaterThan> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? docIntNameId = null, [FromQuery] long? integerValue = null, [FromQuery] long? filterGroupId = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading all documents integer greater than",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_docs_int_grtr_than", parameters);
            },
            result =>
            {
