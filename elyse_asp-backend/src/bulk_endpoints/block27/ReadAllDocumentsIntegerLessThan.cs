...
// Returns metadata for documents and associated files where the designated integer field is less than the given value.
// No filter group is applied.
[Route("api/document/integer/less-than")]
[ApiController]
public class ReadAllDocumentsIntegerLessThan : BaseStoredProcedureController
{
    public ReadAllDocumentsIntegerLessThan(StoredProcedureService storedProcedureService, ILogger<ReadAllDocumentsIntegerLessThan> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? docIntNameId = null, [FromQuery] long? integerValue = null, [FromQuery] long? filterGroupId = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading all documents integer less than",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_docs_int_less_than", parameters);
            },
            result =>
            {
