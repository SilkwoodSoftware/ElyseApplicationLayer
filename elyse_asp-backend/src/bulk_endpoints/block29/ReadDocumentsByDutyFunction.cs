...
// Selects metadata for documents linked to a given duty function.
// Also returns metadata for the files linked to the returned documents.
// A filter group is applied.
[Route("api/documents/duty-function")]
[ApiController]
public class ReadDocumentsByDutyFunction : BaseStoredProcedureController
{
    public ReadDocumentsByDutyFunction(StoredProcedureService storedProcedureService, ILogger<ReadDocumentsByDutyFunction> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> Read([FromQuery] long? functionId = null, [FromQuery] long? filterGroupId = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading documents by duty function",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_docs_by_duty_function", parameters);
            },
            result =>
            {
