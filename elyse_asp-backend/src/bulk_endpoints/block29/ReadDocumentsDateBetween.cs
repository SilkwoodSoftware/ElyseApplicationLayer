...
// Returns metadata for documents and associated files where a date record is between given dates.
// Applies a filter group.
[Route("api/documents/date/between")]
[ApiController]
public class ReadDocumentsDateBetween : BaseStoredProcedureController
{
    public ReadDocumentsDateBetween(StoredProcedureService storedProcedureService, ILogger<ReadDocumentsDateBetween> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> Read([FromQuery] long? docDateNameId = null, [FromQuery] string earlierDate = null, [FromQuery] string laterDate = null, [FromQuery] long? filterGroupId = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading documents date between",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_docs_date_between", parameters);
            },
            result =>
            {
