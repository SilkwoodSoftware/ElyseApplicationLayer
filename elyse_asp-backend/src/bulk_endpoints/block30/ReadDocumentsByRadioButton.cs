...
// Selects a list of documents for a given document radio button attribute ID.
[Route("api/documents/radio-button")]
[ApiController]
public class ReadDocumentsByRadioButton : BaseStoredProcedureController
{
    public ReadDocumentsByRadioButton(StoredProcedureService storedProcedureService, ILogger<ReadDocumentsByRadioButton> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> Read([FromQuery] long? docRadioButtonAttributeId = null, [FromQuery] long? filterGroupId = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading documents by radio button",
            async () =>
            {
                var parameters = new Dictionary<string, object>
                {
                    { "@docradiobattrid", docRadioButtonAttributeId ?? (object)DBNull.Value },
                    { "@filtergroupid", filterGroupId ?? (object)DBNull.Value },
                    { "@formid", formId ?? (object)DBNull.Value }
                };

                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_docs_by_radio_button", parameters);
            },
            result =>
            {
