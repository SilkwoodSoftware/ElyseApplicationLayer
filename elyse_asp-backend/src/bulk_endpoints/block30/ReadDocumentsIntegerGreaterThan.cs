...
// Returns metadata for documents and associated files where the designated integer field is greater than a given value.
[Route("api/documents/integer/greater-than")]
[ApiController]
public class ReadDocumentsIntegerGreaterThan : BaseStoredProcedureController
{
    public ReadDocumentsIntegerGreaterThan(StoredProcedureService storedProcedureService, ILogger<ReadDocumentsIntegerGreaterThan> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? docIntNameId = null, [FromQuery] long? integerValue = null, [FromQuery] long? filterGroupId = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading documents integer greater than",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_docs_int_greater_than", parameters);
            },
            result =>
            {
