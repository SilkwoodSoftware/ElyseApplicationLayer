...
// Returns metadata for documents and associated files where the designated integer field is less than a given value.
// A filter group is applied.
[Route("api/documents/integer/less-than")]
[ApiController]
public class ReadDocumentsIntegerLessThan : BaseStoredProcedureController
{
    public ReadDocumentsIntegerLessThan(StoredProcedureService storedProcedureService, ILogger<ReadDocumentsIntegerLessThan> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? docIntNameId = null, [FromQuery] long? integerValue = null, [FromQuery] long? filterGroupId = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading documents integer less than",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_docs_int_less_than", parameters);
            },
            result =>
            {
