...
// Returns metadata for files where a date record is before a given date.
// No filter group is applied.
[Route("api/file/date/before")]
[ApiController]
public class ReadAllFilesByDateBefore : BaseStoredProcedureController
{
    public ReadAllFilesByDateBefore(StoredProcedureService storedProcedureService, ILogger<ReadAllFilesByDateBefore> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> Read([FromQuery] long? fileDateNameId = null, [FromQuery] string dateValue = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading all files by date before",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_files_date_before", parameters);
            },
            result =>
            {
