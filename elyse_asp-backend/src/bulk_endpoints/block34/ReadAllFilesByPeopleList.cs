...
// Returns metadata for files for a given people list.
// No filter group is applied.
[Route("api/file/people-list")]
[ApiController]
public class ReadAllFilesByPeopleList : BaseStoredProcedureController
{
    public ReadAllFilesByPeopleList(StoredProcedureService storedProcedureService, ILogger<ReadAllFilesByPeopleList> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? peopleListId = null, [FromQuery] long? formId = null)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading all files by people list",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_files_by_pple_list", parameters);
            },
            result =>
            {
