...
// Returns metadata for files where a linked to a given people list.
// A filter group is applied.
[Route("api/files/people-list")]
[ApiController]
public class ReadFilesByPeopleList : BaseStoredProcedureController
{
    public ReadFilesByPeopleList(StoredProcedureService storedProcedureService, ILogger<ReadFilesByPeopleList> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> Read([FromQuery] ReadFilesByPeopleListRequest request)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading files by people list",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_files_by_people_list", parameters);
            },
            result =>
            {
...
public class ReadFilesByPeopleListRequest
{
