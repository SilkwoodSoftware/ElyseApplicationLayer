...
// Returns metadata for files where a date record is equal to a given real number value.
// Applies a filter group.
[Route("api/files/real/equals")]
[ApiController]
public class ReadFilesRealEquals : BaseStoredProcedureController
{
    public ReadFilesRealEquals(StoredProcedureService storedProcedureService, ILogger<ReadFilesRealEquals> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> Read([FromQuery] ReadFilesRealEqualsRequest request)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading files real equals",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_files_real_equals", parameters);
            },
            result =>
            {
...
public class ReadFilesRealEqualsRequest
{
