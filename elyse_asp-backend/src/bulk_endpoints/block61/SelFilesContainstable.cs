...
// Selects file metadata for all the files which contain the given text using the CONTAINSTABLE function as applied to the stored plain text content of the file.    A filter group is applied.
[Route("api/files-containstable")]
[ApiController]
public class SelFilesContainstable : BaseStoredProcedureController
{
    public SelFilesContainstable(StoredProcedureService storedProcedureService, ILogger<SelFilesContainstable> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> GetFilesContainstable([FromQuery] string containsString, [FromQuery] long? filterGroupId, [FromQuery] long? formId)
    {
