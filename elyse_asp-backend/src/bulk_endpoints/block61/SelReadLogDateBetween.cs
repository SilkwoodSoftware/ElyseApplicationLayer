...
// Selects records from the file read log where the where the date is between given dates.
[Route("api/read-log-date-between")]
[ApiController]
public class SelReadLogDateBetween : BaseStoredProcedureController
{
    public SelReadLogDateBetween(StoredProcedureService storedProcedureService, ILogger<SelReadLogDateBetween> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> GetReadLogDateBetween([FromQuery] string earlierDate, [FromQuery] string laterDate)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving read log by date range",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("controlling.usp_SEL_read_log_date_between", parameters);
            },
            result =>
            {
