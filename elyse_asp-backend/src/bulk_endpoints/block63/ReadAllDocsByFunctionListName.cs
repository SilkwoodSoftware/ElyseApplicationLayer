...
// Selects all documents by function list name.
...
[Route("api/all-documents/function-list-name")]
[ApiController]
public class ReadAllDocsByFunctionListName : BaseStoredProcedureController
{
    public ReadAllDocsByFunctionListName(StoredProcedureService storedProcedureService, ILogger<ReadAllDocsByFunctionListName> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving all documents by function list name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_dc_by_fnct_lst_nm", parameters);
            },
            result =>
            {
