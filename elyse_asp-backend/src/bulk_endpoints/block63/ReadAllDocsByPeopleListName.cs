...
// Selects all documents by people list name.
...
[Route("api/all-documents/people-list-name")]
[ApiController]
public class ReadAllDocsByPeopleListName : BaseStoredProcedureController
{
    public ReadAllDocsByPeopleListName(StoredProcedureService storedProcedureService, ILogger<ReadAllDocsByPeopleListName> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving all documents by people list name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_dc_by_ppl_lst_nm", parameters);
            },
            result =>
            {
