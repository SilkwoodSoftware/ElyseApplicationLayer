...
// Selects all files by function list name.
...
[Route("api/all-files/function-list-name")]
[ApiController]
public class ReadAllFilesByFunctionListName : BaseStoredProcedureController
{
    public ReadAllFilesByFunctionListName(StoredProcedureService storedProcedureService, ILogger<ReadAllFilesByFunctionListName> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving all files by function list name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_fl_by_fnct_lst_nm", parameters);
            },
            result =>
            {
