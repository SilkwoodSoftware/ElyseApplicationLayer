...
// Selects all files by functional field name.
...
[Route("api/all-files/functional-field-name")]
[ApiController]
public class ReadAllFilesByFunctionalFieldName : BaseStoredProcedureController
{
    public ReadAllFilesByFunctionalFieldName(StoredProcedureService storedProcedureService, ILogger<ReadAllFilesByFunctionalFieldName> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving all files by functional field name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_fl_by_funct_fld_nm", parameters);
            },
            result =>
            {
