...
// Selects all files by people list name.
...
[Route("api/all-files/people-list-name")]
[ApiController]
public class ReadAllFilesByPeopleListName : BaseStoredProcedureController
{
    public ReadAllFilesByPeopleListName(StoredProcedureService storedProcedureService, ILogger<ReadAllFilesByPeopleListName> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving all files by people list name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_fl_by_ppl_lst_nm", parameters);
            },
            result =>
            {
