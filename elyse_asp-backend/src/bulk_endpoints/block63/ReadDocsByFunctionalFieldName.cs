...
// Selects documents by functional field name.
...
[Route("api/documents/functional-field-name")]
[ApiController]
public class ReadDocsByFunctionalFieldName : BaseStoredProcedureController
{
    public ReadDocsByFunctionalFieldName(StoredProcedureService storedProcedureService, ILogger<ReadDocsByFunctionalFieldName> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? filterGroupId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving documents by functional field name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_docs_by_funct_fld_name", parameters);
            },
            result =>
            {
