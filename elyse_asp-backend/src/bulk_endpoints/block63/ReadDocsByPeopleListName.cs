...
// Selects documents by people list name.
...
[Route("api/documents/people-list-name")]
[ApiController]
public class ReadDocsByPeopleListName : BaseStoredProcedureController
{
    public ReadDocsByPeopleListName(StoredProcedureService storedProcedureService, ILogger<ReadDocsByPeopleListName> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? filterGroupId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving documents by people list name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_dc_by_ppl_list_name", parameters);
            },
            result =>
            {
