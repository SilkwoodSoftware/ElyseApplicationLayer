...
// Selects files by function list name.
...
[Route("api/files/function-list-name")]
[ApiController]
public class ReadFilesByFunctionListName : BaseStoredProcedureController
{
    public ReadFilesByFunctionListName(StoredProcedureService storedProcedureService, ILogger<ReadFilesByFunctionListName> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? filterGroupId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving files by function list name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_fl_by_fnct_list_name", parameters);
            },
            result =>
            {
