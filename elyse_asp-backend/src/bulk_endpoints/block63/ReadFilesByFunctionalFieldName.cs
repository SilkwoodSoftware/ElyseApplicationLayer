...
// Selects files by functional field name.
...
[Route("api/files/functional-field-name")]
[ApiController]
public class ReadFilesByFunctionalFieldName : BaseStoredProcedureController
{
    public ReadFilesByFunctionalFieldName(StoredProcedureService storedProcedureService, ILogger<ReadFilesByFunctionalFieldName> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? filterGroupId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving files by functional field name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_files_by_fnct_fld_name", parameters);
            },
            result =>
            {
