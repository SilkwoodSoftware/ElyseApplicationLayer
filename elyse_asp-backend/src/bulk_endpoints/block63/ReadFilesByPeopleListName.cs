...
// Selects files by people list name.
...
[Route("api/files/people-list-name")]
[ApiController]
public class ReadFilesByPeopleListName : BaseStoredProcedureController
{
    public ReadFilesByPeopleListName(StoredProcedureService storedProcedureService, ILogger<ReadFilesByPeopleListName> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? filterGroupId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving files by people list name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_fl_by_ppl_list_name", parameters);
            },
            result =>
            {
