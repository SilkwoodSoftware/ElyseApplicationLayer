...
// Selects file metadata for all the files which contain the given text using the CONTAINS function and applied to the stored plain text content of the file.  A filter group is applied.
[Route("api/files-contains")]
[ApiController]
public class SelFilesContainsWithDocs : BaseStoredProcedureController
{
    public SelFilesContainsWithDocs(StoredProcedureService storedProcedureService, ILogger<SelFilesContains> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read-with-docs")]
    public async Task<IActionResult> GetFilesContainsWithDocs([FromQuery] string containsString, [FromQuery] long? filterGroupId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving files containing text",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_files_contains", parameters);
            },
            result =>
            {
