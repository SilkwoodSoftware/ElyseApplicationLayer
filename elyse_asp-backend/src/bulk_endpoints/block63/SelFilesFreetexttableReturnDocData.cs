...
// Selects file metadata for all the files which contain the given text using the FREETEXTTABLE function as applied to the stored plain text content of the file.    A filter group is applied.
[Route("api/files-freetexttable")]
[ApiController]
public class SelFilesFreetexttableWithDocs : BaseStoredProcedureController
{
    public SelFilesFreetexttableWithDocs(StoredProcedureService storedProcedureService, ILogger<SelFilesFreetexttable> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read-with-docs")]
    public async Task<IActionResult> GetFilesFreetexttableWithDocs([FromQuery] string containsString, [FromQuery] long? filterGroupId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "retrieving files using freetexttable search",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_files_freetexttable", parameters);
            },
            result =>
            {
