...
// Selects metadata for all documents linked to the connected user.
// No filter group is applied.
[Route("api/all-documents/connected-user")]
[ApiController]
public class ReadAllDocsForUser : BaseStoredProcedureController
{
    public ReadAllDocsForUser(StoredProcedureService storedProcedureService, ILogger<ReadAllDocsForUser> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> Read([FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading all documents for connected user",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_docs_for_user", parameters);
            },
            result =>
            {
