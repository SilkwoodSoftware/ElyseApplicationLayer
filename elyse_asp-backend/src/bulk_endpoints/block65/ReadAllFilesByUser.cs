...
// Returns metadata for files linked to a given user.
// No filter group is applied.
[Route("api/all-files/user")]
[ApiController]
public class ReadAllFilesByUser : BaseStoredProcedureController
{
    public ReadAllFilesByUser(StoredProcedureService storedProcedureService, ILogger<ReadAllFilesByUser> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? userId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            ...
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_files_by_user", parameters);
            },
            result =>
            {
