...
// Returns metadata for files linked to the connected user.
// No filter group is applied.
[Route("api/all-files/connected-user")]
[ApiController]
public class ReadAllFilesForUser : BaseStoredProcedureController
{
    public ReadAllFilesForUser(StoredProcedureService storedProcedureService, ILogger<ReadAllFilesForUser> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading all files for connected user",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_all_files_for_user", parameters);
            },
            result =>
            {
