...
[Route("api/files/user-field-name")]
[ApiController]
public class ReadFilesByUserFgn : BaseStoredProcedureController
{
    public ReadFilesByUserFgn(StoredProcedureService storedProcedureService, ILogger<ReadFilesByUserFgn> logger)
    ...
    public async Task<IActionResult> Read([FromQuery] long? generalFieldNameId, [FromQuery] long? filterGroupId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading files by user field name",
            async () =>
            {
                ...
                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_files_by_user_fgn", parameters);
            },
            result =>
            {
