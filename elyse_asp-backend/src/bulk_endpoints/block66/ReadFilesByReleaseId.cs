...
// Returns metadata for files for a given release ID.
// Filter group is applied.
[Route("api/files/release-id")]
[ApiController]
public class ReadFilesByReleaseId : BaseStoredProcedureController
{
    public ReadFilesByReleaseId(StoredProcedureService storedProcedureService, ILogger<ReadFilesByReleaseId> logger)
        : base(storedProcedureService, logger, null)
    {
    }

    [HttpGet("read")]
    public async Task<IActionResult> Read([FromQuery] long? releaseId, [FromQuery] long? filterGroupId, [FromQuery] long? formId)
    {
        return await ExecuteWithErrorHandlingAsync(
            "reading files by release ID",
            async () =>
            {
                var parameters = new Dictionary<string, object>
                {
                    { "@releaseid", releaseId ?? (object)DBNull.Value },
                    { "@filtergroupid", filterGroupId ?? (object)DBNull.Value },
                    { "@formid", formId ?? (object)DBNull.Value }
                };

                return await _storedProcedureService.ExecuteStoredProcedureAsync("reading.usp_SEL_files_by_release_id", parameters);
            },
            result =>
            {
