...
public abstract class BaseStoredProcedureController : ControllerBase
{
    ...
    protected List<Dictionary<string, object>> TransformDocumentData(List<Dictionary<string, object>> documentData)
    {
        ...
        // Check if Document ID column exists with actual values in the data
        bool hasDocumentIdColumn = documentData.Any(row =>
            row.ContainsKey("Document ID") &&
            row["Document ID"] != null &&
            row["Document ID"] != DBNull.Value &&
            !string.IsNullOrEmpty(row["Document ID"].ToString()));

        // Check if File ID column exists as a direct column in the data (regardless of values)
        bool hasFileIdColumn = documentData.Any(row => row.ContainsKey("File ID"));

        // Check if File ID exists as a Mnemonic/Value pair in the data
        bool hasFileIdMnemonic = documentData.Any(row =>
            row.ContainsKey("Mnemonic") && row["Mnemonic"] != null &&
            row["Mnemonic"].ToString() == "File ID");

        // Include "Document ID" only if the column exists with actual values
        if (hasDocumentIdColumn)
        {
            uniqueColumns["Document ID"] = ("Document ID", "Document identifier");
        }
        
        // Include "File ID" if the column exists or if it appears as a Mnemonic, even if all values are empty
        if (hasFileIdColumn || hasFileIdMnemonic)
        {
        ...
        foreach (var group in groupedData)
        {
            ...
            // Set the "Document ID" as the first column only if it exists in the data
            if (hasDocumentIdColumn)
            {
                var docIdRow = group.FirstOrDefault(r =>
                    r.ContainsKey("Document ID") &&
                    r["Document ID"] != null &&
                    r["Document ID"] != DBNull.Value &&
                    !string.IsNullOrEmpty(r["Document ID"].ToString()));
                string documentIdValue = docIdRow != null ? docIdRow["Document ID"].ToString() : "";
           ...
           // Find and set the "File ID" only if it exists in uniqueColumns
            if (uniqueColumns.ContainsKey("File ID"))
            {
                ...
                if (hasFileIdColumn)
                {
                    // File ID exists as a direct column - find a row where it has an actual value
                    var fileIdRow = group.FirstOrDefault(r =>
                        r.ContainsKey("File ID") &&
                        r["File ID"] != null &&
                        r["File ID"] != DBNull.Value &&
                        !string.IsNullOrEmpty(r["File ID"].ToString()));
                    if (fileIdRow != null)
                    {
                        fileIdValue = fileIdRow["File ID"].ToString();
                    }
                }
                
                if (string.IsNullOrEmpty(fileIdValue) && hasFileIdMnemonic)
                {
                    // File ID exists as a Mnemonic/Value pair - extract the value
                    var fileIdMnemonicRow = group.FirstOrDefault(r =>
                        r.ContainsKey("Mnemonic") && r["Mnemonic"] != null &&
                        r["Mnemonic"].ToString() == "File ID" &&
                        r.ContainsKey("Value") && r["Value"] != null && r["Value"] != DBNull.Value);
                    if (fileIdMnemonicRow != null)
                    {
                        fileIdValue = fileIdMnemonicRow["Value"].ToString();
                    }
                }
