...
namespace elyse_asp_backend.Version
{
    ...
    public class BackendVersionController : ControllerBase
    {
        ...
        public IActionResult GetVersion()
        {
            ...
            // Get version from assembly metadata (single source of truth: .csproj)
            var version = metadata.FirstOrDefault(a => a.Key == "Version")?.Value ?? "unknown";
            
            // Get build ID from assembly metadata
            var buildId = metadata.FirstOrDefault(a => a.Key == "BuildId")?.Value ?? "unknown";
            
            // Get license from assembly metadata
            var license = metadata.FirstOrDefault(a => a.Key == "License")?.Value ?? "unknown";
